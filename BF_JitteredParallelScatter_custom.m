function [ff,xx] = BF_JitteredParallelScatter_custom(dataCell,addMeans,doveTail,makeFigure,extraParams)
% Custom version of BF_JitteredParallelScatter from hctsa toolbox
%
...
if doveTail
    ...
    for i = 1:numGroups
        ...
        x = x(keepR);
        f = f(keepR);
        plot(customOffset+i+f*offsetRange/2,x,'-','color',theColors{i},'LineWidth',1)
        plot(customOffset+i-f*offsetRange/2,x,'-','color',theColors{i},'LineWidth',1)
