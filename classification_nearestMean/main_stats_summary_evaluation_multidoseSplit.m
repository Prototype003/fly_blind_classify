...
for d = 1 : length(dsets)
    for pair = 1 : size(stats.(dsets{d}).(perf_type).condition_pairs, 1)
        ...
        subplot(sp_rows, sp_cols, sp_counter);
        imagesc(tmp, [0.4 0.6]); c = colorbar;
        title([...
            perf_type ' '...
            dsets{d} newline...
...
sp_counter = 1;
m = max(sig_all_count(:));
mPlots = 16; % m plots, plus two more
for count = 1 : m
...
dsets_specific = {'train', 'multidose8', 'multidose4', 'singledose', 'sleep'};
pairs_specific = {[1], [1], [1], [1], [3]};
...
sp_counter = 1;
m = max(sig_all_count(:));
mPlots = 9; % m plots, plus two more
for count = 1 : m
...
% TODO - create function which gets condition pairs which include only the
%   specified conditions
dsets_specific = {'multidose8', 'multidose4', 'singledose', 'sleep'};
pairs_specific = {[1], [1], [1], [1], [3]};
...
for d = 1 : length(dsets_specific)
    for pair = 1 : length(pairs_specific{d})%size(stats.(dsets{d}).(perf_type).condition_pairs, 1)
        ...
        tmp = stats.([dsets_specific{d} dset_suffixes{d}]).(perf_type).sig(:, :, pairs_specific{d}(pair));
        sig_all = sig_all & tmp;
        sig_all_count = sig_all_count + tmp;
...
% Not needed - this is just a subset of the full histograms plotted in a
% previous section 
...
[B, I] = sort(perfs);
sig_list = find(sig_all(ch, :));
disp(['f' num2str(sig_list(I(end))) ' perf=' num2str(B(end))]);% Best feature from the dataset
