...
Plot DNV matrix for all datasets

NOTE - this code was cannibalised
ORIGINALLY was supposed to plot flies x (sig-features x channels)
But was modified to try plot flies x features (averaged across channels)
SO it includes a mix of code for trying to achieve both, and code for
the original plot was updated
SO it needs to be updated to achieve original plot
...
best_fId = 7701; % highest average performance across evaluation flies from excel pivot table
best_ch = 1; % it's actually 3
best_fName = 'MD_rawHRVmeas_SD1';
...
if remove_md == 1
	...
	yticklabels = dsets([1 4 5]);
	
	% Reobtain updated ytickpos
	ytickpos = ones(size(yticklabels));
	for d = 1 : length(yticklabels) - 1

		[~, nFlies, ~, nEpochs] = getDimensionsFast(yticklabels{d});
...
xtickpos = ones(nCh, 1);
for ch = 1 : nCh-1
	xtickpos(ch+1) = xtickpos(ch) + numel(fIds{ch});
end
xticklabels = num2cell((1:nCh));
xticklabels = cellfun(@(x) ['ch' num2str(x)], xticklabels, 'UniformOutput', false);

% Add position of selected feature
xtickpos = [xtickpos(1:best_ch); xtickpos(best_ch)-1+find(fIds{best_ch}(ch_featureOrders{best_ch})==best_fId); xtickpos(best_ch+1:end)];
xticklabels = [xticklabels(1:best_ch) {['f' num2str(best_fId)]} xticklabels(best_ch+1:end)];

% Add total number of features at the end
xtickpos = [xtickpos; size(vis_mat, 2)];
xticklabels = [xticklabels {num2str(size(vis_mat, 2))}];
...
imagesc(vis_mat(:, fOrder));
title(['differences in normalised values' newline 'ch' num2str(best_ch) ' f' num2str(best_fId) ' ' best_fName], 'interpreter', 'none');
c = colorbar;
title(c, 'DNV');
